...
def create_vector_db(source_folder):
    ...
    loader = GenericLoader.from_filesystem(
        ...
        glob="**/src/main/java/**/*",
        suffixes=[".java"],
        parser=LanguageParser(language=Language.JAVA, parser_threshold=JAVA_PARSER_THRESHOLD),
    )
    docs = loader.load()
    print("Loaded : ", len(docs), "documents")
    for doc in docs:
        metadata = doc.metadata
        java_source = metadata['source']
        json_source = java_source.replace(".java", ".json")
        json_metadata = load_json(json_source)
        for key, value in json_metadata.items():
            if isinstance(value, list):
                json_metadata[key] = ', '.join(map(str, value))
        doc.metadata.update(json_metadata)
        print("AAAA:", doc.metadata)
